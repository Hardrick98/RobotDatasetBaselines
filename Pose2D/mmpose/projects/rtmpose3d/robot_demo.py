...
def process_one_image(args, bboxes:np.ndarray,frame: np.ndarray, frame_idx: int,
                      pose_estimator,
...
def main():
    ...
    annotations = joblib.load("/work/ToyotaHPE/rcatalini/EventRobotPose/ego1L_annotations.pkl")
    tokens =args.input.split('/')
    robot = tokens[-3]
    scene = tokens[-4]
    frame_idx = int(tokens[-1].split("_")[1].split(".")[0])
    
    
    ann = annotations[robot][scene]
    bb2D = ann["bb2D"][frame_idx,0]  
    x1, y1, x2, y2 = bb2D

    bb2D = np.array([float(x1), float(y1), float(x2), float(y2)]).reshape(1,4)
    ...
    if input_type == 'image':
        ...
        while video.isOpened():
            ...
            pred_3d_instances = pred_3d_instances.to_dict()
